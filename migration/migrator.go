...
func NewMigrator(migrationFolder string, db *gorm.DB) *Migrator {
	// 初始化必要属性
	migrator := &Migrator{
...
func (migrator *Migrator) createMigrationsTable() {
	...
	if !migrator.Migrator.HasTable(&migration) {
		if err := migrator.Migrator.CreateTable(&migration); err != nil {
			console.Error("CreateTable error: " + err.Error())
		}
	}
}
...
func (migrator *Migrator) Fresh() {
	// 获取数据库名称，用以提示
	dbname := CurrentDatabase(migrator.DB)

	// 删除所有表
	err := DeleteAllTables("mysql", migrator.DB)
	console.ExitIf(err)
